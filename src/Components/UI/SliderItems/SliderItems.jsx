...
const SliderItems = () => {
  ...
  useEffect(() => {
    axios
      .get("https://gospodin-rusan.onrender.com/catalog?&_limit=16")
      // .then(resp => resp.json())
      .then((resp) => {
        setCards(resp.data);
      });
  ...
  return (
    <div className="Slider">
          ...
          {cards.map((item) => {
            if (item.category.includes("Скидки")) {
              return (
                <div
                  key={item.id}
                  className="coffee__cards-inner-row-card slider-card"
                >
      ...
      <button
        className="Slider__button_prev"
        onClick={(e) => prevHandler(e.target.previousElementSibling.firstChild)}
      >{`<`}</button>
      <button
        className="Slider__button_next"
        onClick={(e) =>
          nextHandler(
