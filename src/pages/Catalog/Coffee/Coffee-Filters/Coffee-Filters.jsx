...
const CoffeeFilters = () => {
  const [filters, setFilters] = useState({roastlevel: [], coffeefrom: [], kislinka: [], pmethod: [], special: [], coffeetype: [], cmethod: [], coffeelevel: []})
  const [key, setKey] = useState(0)
  const [tum, setTum] = useState(false)
  const oBack = tum ? '#f9b300' : '#e4e4e4'
  const filterFun = (e, filt)=>{
    let et = e.target.localName == 'img' ? e.target.previousElementSibling.previousElementSibling : e.target
    let index = filters[filt].indexOf(et.nextSibling.innerText)
    setTum(!tum)
    if(et.style.background == 'rgb(249, 179, 0)' && filters[filt].includes(et.nextSibling.innerText) == true){
      filters[filt].splice(index, 1)
    }
    if(filters[filt].includes(et.nextSibling.innerText) === false && et.style.background == 'rgb(228, 228, 228)'){
            ...
            <div className="filters-function-left">
              ...
              <div className="filters-function-left-sec">
                ...
                <div className="fiveCoffees">
                  <span onClick={e=>handleCoffeelevel(e)} className="white-circle"></span>
            ...
            images.map(item => (
              <div key={item.id} className="filters-bottom-blocks-item">
                <div className="filters-bottom-blocks-item-inner">
                  ...
                  <p>{item.text}</p>
                  <img onClick={(e)=>handleCmethod(e)} src={item.image} alt="" />
                </div>
              </div>
            ))
          }
